...
From MetaCoq.PCUIC Require Export PCUICEquality.
From MetaCoq.GCIC Require Import GCICAst GCICAstUtils GCICInduction
     GCICReflect GCICLiftSubst.
...
| eq_Rel n  :
    eq_term_upto_univ Re Rle (gRel n) (gRel n)

| eq_Evar e args args' :
    All2 (eq_term_upto_univ Re Re) args args' ->
    eq_term_upto_univ Re Rle (gEvar e args) (gEvar e args')

| eq_Var id :
    eq_term_upto_univ Re Rle (gVar id) (gVar id)

| eq_Sort s s' :
    Rle s s' ->
    eq_term_upto_univ Re Rle (gSort s) (gSort s')

| eq_App t t' u u' :
    eq_term_upto_univ Re Rle t t' ->
    eq_term_upto_univ Re Re u u' ->
    eq_term_upto_univ Re Rle (gApp t u) (gApp t' u')

| eq_Const c u u' :
    R_universe_instance Re u u' ->
    eq_term_upto_univ Re Rle (gConst c u) (gConst c u')

| eq_Ind i u u' :
    R_universe_instance Re u u' ->
    eq_term_upto_univ Re Rle (gInd i u) (gInd i u')

| eq_Construct i k u u' :
    R_universe_instance Re u u' ->
    eq_term_upto_univ Re Rle (gConstruct i k u) (gConstruct i k u')

| eq_Lambda na na' ty ty' t t' :
    eq_term_upto_univ Re Re ty ty' ->
    eq_term_upto_univ Re Rle t t' ->
    eq_term_upto_univ Re Rle (gLambda na ty t) (gLambda na' ty' t')

| eq_Prod na na' a a' b b' :
    eq_term_upto_univ Re Re a a' ->
    eq_term_upto_univ Re Rle b b' ->
    eq_term_upto_univ Re Rle (gProd na a b) (gProd na' a' b')

| eq_LetIn na na' t t' ty ty' u u' :
    eq_term_upto_univ Re Re t t' ->
    eq_term_upto_univ Re Re ty ty' ->
    eq_term_upto_univ Re Rle u u' ->
    eq_term_upto_univ Re Rle (gLetIn na t ty u) (gLetIn na' t' ty' u')
...
| eq_Case indn p p' c c' brs brs' :
    ...
    eq_term_upto_univ Re Rle (gCase indn p c brs) (gCase indn p' c' brs')

| eq_Proj p c c' :
    eq_term_upto_univ Re Re c c' ->
    eq_term_upto_univ Re Rle (gProj p c) (gProj p c')
...
| eq_Fix mfix mfix' idx :
    ...
    eq_term_upto_univ Re Rle (gFix mfix idx) (gFix mfix' idx)
...
| eq_CoFix mfix mfix' idx :
    ...
    eq_term_upto_univ Re Rle (gCoFix mfix idx) (gCoFix mfix' idx)
| eq_Ukn A A':
    eq_term_upto_univ Re Re A A' ->
    eq_term_upto_univ Re Rle (gUkn A) (gUkn A')

| eq_Cast A A' t t' :
    eq_term_upto_univ Re Rle A A' ->
    eq_term_upto_univ Re Re t t' ->
    eq_term_upto_univ Re Rle (gCast A t) (gCast A' t').
...
Fixpoint eqb_term_upto_univ (equ lequ : Universe.t -> Universe.t -> bool) (u v : term) : bool :=
  ...
  | gConstruct i k u, gConstruct i' k' u' =>
    eqb i i' &&
    eqb k k' &&
    forallb2 equ (map Universe.make u) (map Universe.make u')

  | gLambda na A t, gLambda na' A' t' =>
    eqb_term_upto_univ equ equ A A' &&
    eqb_term_upto_univ equ lequ t t'

  | gProd na A B, gProd na' A' B' =>
    eqb_term_upto_univ equ equ A A' &&
    eqb_term_upto_univ equ lequ B B'

  | gLetIn na B b u, gLetIn na' B' b' u' =>
    eqb_term_upto_univ equ equ B B' &&
    eqb_term_upto_univ equ equ b b' &&
    eqb_term_upto_univ equ lequ u u'

  | gCase indp p c brs, gCase indp' p' c' brs' =>
    eqb indp indp' &&
  ...
  | gCoFix mfix idx, gCoFix mfix' idx' =>
    eqb idx idx' &&
    forallb2 (fun x y =>
      eqb_term_upto_univ equ equ x.(dtype) y.(dtype) &&
      eqb_term_upto_univ equ equ x.(dbody) y.(dbody) &&
  ...
  | gCast A t, gCast A' t' =>
    eqb_term_upto_univ equ lequ A A' &&
...
Proof.
  ...
  - cbn - [eqb]. eqspecs. equspec equ he. equspec lequ hle. ih.
    ...
    + destruct l0.
      ...
      * constructor. intro bot. inversion bot.
        inversion X.
    + destruct l0.
...
Local Open Scope gcic.

Definition upto_names := eq_term_upto_univ eq eq.

Infix "≡" := upto_names (at level 60) : gcic.

Infix "≡'" := (eq_term_upto_univ eq eq) (at level 60) : gcic.
