...
From MetaCoq.GCIC Require Import GCICAst GCICAstUtils GCICInduction.
...
Fixpoint rename f t : term :=
  match t with
  | gRel i => gRel (f i)
  | gEvar ev args => gEvar ev (List.map (rename f) args)
  | gLambda na T M => gLambda na (rename f T) (rename (shiftn 1 f) M)
  | gApp u v => gApp (rename f u) (rename f v)
  | gProd na A B => gProd na (rename f A) (rename (shiftn 1 f) B)
  | gLetIn na b t b' => gLetIn na (rename f b) (rename f t) (rename (shiftn 1 f) b')
  | gCase ind p c brs =>
    let brs' := List.map (on_snd (rename f)) brs in
    gCase ind (rename f p) (rename f c) brs'
  | gProj p c => gProj p (rename f c)
  | gFix mfix idx =>
  ...
  | gCoFix mfix idx =>
    let mfix' := List.map (map_def (rename f) (rename (shiftn (List.length mfix) f))) mfix in
    gCoFix mfix' idx
  | gUkn A => gUkn (rename f A)
  | gCast A t => gCast (rename f A) (rename f t)
  | x => x
  end.

Fixpoint lift n k t : term :=
  match t with
  | gRel i => gRel (if Nat.leb k i then n + i else i)
  | gEvar ev args => gEvar ev (List.map (lift n k) args)
  | gLambda na T M => gLambda na (lift n k T) (lift n (S k) M)
  | gApp u v => gApp (lift n k u) (lift n k v)
  | gProd na A B => gProd na (lift n k A) (lift n (S k) B)
  | gLetIn na b t b' => gLetIn na (lift n k b) (lift n k t) (lift n (S k) b')
  | gCase ind p c brs =>
    let brs' := List.map (on_snd (lift n k)) brs in
    gCase ind (lift n k p) (lift n k c) brs'
  | gProj p c => gProj p (lift n k c)
  | gFix mfix idx =>
  ...
  | gCoFix mfix idx =>
    ...
    let mfix' := List.map (map_def (lift n k) (lift n k')) mfix in
    gCoFix mfix' idx
  | gUkn A => gUkn (lift n k A)
  | gCast A t => gCast (lift n k A) (lift n k t)
  | x => x
  end.
...
Fixpoint subst s k u :=
  ...
  | gRel n =>
    if Nat.leb k n then
      ...
      | Some b => lift0 k b
      | None => gRel (n - List.length s)
      end
    else gRel n
  | gEvar ev args => gEvar ev (List.map (subst s k) args)
  | gLambda na T M => gLambda na (subst s k T) (subst s (S k) M)
  | gApp u v => gApp (subst s k u) (subst s k v)
  | gProd na A B => gProd na (subst s k A) (subst s (S k) B)
  | gLetIn na b ty b' => gLetIn na (subst s k b) (subst s k ty) (subst s (S k) b')
  | gCase ind p c brs =>
    let brs' := List.map (on_snd (subst s k)) brs in
    gCase ind (subst s k p) (subst s k c) brs'
  | gProj p c => gProj p (subst s k c)
  | gFix mfix idx =>
    let k' := List.length mfix + k in
  ...
  | gCoFix mfix idx =>
    ...
    let mfix' := List.map (map_def (subst s k) (subst s k')) mfix in
    gCoFix mfix' idx
  | gUkn A => gUkn (subst s k A)
  | gCast A t => gCast (subst s k A) (subst s k t)
  | x => x
  end.
...
Fixpoint closedn k (t : term) : bool :=
  match t with
  | gRel i => Nat.ltb i k
  | gEvar ev args => List.forallb (closedn k) args
  | gLambda _ T M | gProd _ T M => closedn k T && closedn (S k) M
  | gApp u v => closedn k u && closedn k v
  | gLetIn na b t b' => closedn k b && closedn k t && closedn (S k) b'
  | gCase ind p c brs =>
    let brs' := List.forallb (test_snd (closedn k)) brs in
    closedn k p && closedn k c && brs'
  | gProj p c => closedn k c
  | gFix mfix idx =>
    let k' := List.length mfix + k in
  ...
  | gCoFix mfix idx =>
    ...
    List.forallb (test_def (closedn k) (closedn k')) mfix
  | gUkn A => closedn k A
  | gCast A t => closedn k A && closedn k t
  | x => true
  end.
...
Lemma lift_rel_ge :
  forall k n p, p <= n -> lift k p (gRel n) = gRel (k + n).
Proof.
  intros; simpl in |- *.
  now elim (leb_spec p n).
Qed.

Lemma lift_rel_lt : forall k n p, p > n -> lift k p (gRel n) = gRel n.
Proof.
  intros; simpl in |- *.
  now elim (leb_spec p n).
Qed.

Lemma subst_rel_lt : forall u n k, k > n -> subst u k (gRel n) = gRel n.
Proof.
  simpl in |- *; intros.
...
Lemma subst_rel_gt :
  forall u n k, n >= k + length u -> subst u k (gRel n) = gRel (n - length u).
Proof.
  simpl in |- *; intros.
...
Proof.
  ...
  simpl. rewrite (IHl n k (gApp t a)). reflexivity.
Qed.
...
Proof.
  ...
  - simpl lift. f_equal. solve_all. unfold test_def in b. rtoProp. solve_all.
  - simpl lift. f_equal. solve_all. unfold test_def in b. rtoProp. solve_all.
Qed.
...
Fixpoint inst s u :=
  match u with
  | gRel n => s n
  | gEvar ev args => gEvar ev (List.map (inst s) args)
  | gLambda na T M => gLambda na (inst s T) (inst (up 1 s) M)
  | gApp u v => gApp (inst s u) (inst s v)
  | gProd na A B => gProd na (inst s A) (inst (up 1 s) B)
  | gLetIn na b ty b' => gLetIn na (inst s b) (inst s ty) (inst (up 1 s) b')
  | gCase ind p c brs =>
    let brs' := List.map (on_snd (inst s)) brs in
    gCase ind (inst s p) (inst s c) brs'
  | gProj p c => gProj p (inst s c)
  | gFix mfix idx =>
  ...
  | gCoFix mfix idx =>
    let mfix' := map (map_def (inst s) (inst (up (List.length mfix) s))) mfix in
    gCoFix mfix' idx
  | gUkn A => gUkn (inst s A)
  | gCast A t => gCast (inst s A) (inst s t)
  | x => x
  end.
...
Definition shift : nat -> term := gRel ∘ S.
...
Definition Up σ : substitution := gRel 0 ⋅ (σ ∘s ↑).
...
Definition shiftk (k : nat) (x : nat) := gRel (k + x).
...
Lemma idsn_lt {n i} : i < n -> nth_error (idsn n) i = Some (gRel i).
Proof.
...
Lemma nth_error_idsn_Some :
  forall n k,
    k < n ->
    nth_error (idsn n) k = Some (gRel k).
Proof.
  intros n k h.
  induction n in k, h |- *.
  - inversion h.
...
Lemma inst_app {s t σ} : (gApp s t).[σ] = gApp s.[σ] t.[σ].
Proof. reflexivity. Qed.

Lemma inst_lam {na t b σ} : (gLambda na t b).[σ] = gLambda na t.[σ] b.[⇑ σ].
Proof.
  simpl. now rewrite up_Up.
Qed.

Lemma inst_prod {na t b σ} : (gProd na t b).[σ] = gProd na t.[σ] b.[⇑ σ].
Proof.
  simpl. now rewrite up_Up.
Qed.

Lemma inst_letin {na t b b' σ} : (gLetIn na t b b').[σ] = gLetIn na t.[σ] b.[σ] b'.[⇑ σ].
Proof.
  simpl. now rewrite up_Up.
Qed.

Lemma inst_fix {mfix idx σ} : (gFix mfix idx).[σ] =
                              gFix (map (map_def (inst σ) (inst (⇑^#|mfix| σ))) mfix) idx.
Proof.
  simpl. f_equal. apply map_ext. intros x. apply map_def_eq_spec. reflexivity.
  now rewrite up_Upn.
Qed.

Lemma inst_cofix {mfix idx σ} : (gCoFix mfix idx).[σ] =
                                gCoFix (map (map_def (inst σ) (inst (⇑^#|mfix| σ))) mfix) idx.
Proof.
  simpl. f_equal. apply map_ext. intros x. apply map_def_eq_spec. reflexivity.
  now rewrite up_Upn.
Qed.
...
Lemma subst_cons_0 t σ : (gRel 0).[t ⋅ σ] = t. Proof. reflexivity. Qed.
...
Lemma subst_cons_ren i f : (gRel i ⋅ ren f) =1 ren (subst_cons_gen i f).
Proof.
...
Lemma subst_cons_0_shift : gRel 0 ⋅ ↑ =1 ids.
...
Lemma term_forall_ctx_list_ind :
  forall P : context -> term -> Type,
    (forall Γ (n : nat), P Γ (gRel n)) ->
    (forall Γ (i : ident), P Γ (gVar i)) ->
    (forall Γ (n : nat) (l : list term), All (P Γ) l -> P Γ (gEvar n l)) ->
    (forall Γ s, P Γ (gSort s)) ->
    (forall Γ (n : name) (t : term), P Γ t -> forall t0 : term, P (vass n t :: Γ) t0 -> P Γ (gProd n t t0)) ->
    (forall Γ (n : name) (t : term), P Γ t -> forall t0 : term, P (vass n t :: Γ) t0 -> P Γ (gLambda n t t0)) ->
    (forall Γ (n : name) (t : term),
        P Γ t -> forall t0 : term, P Γ t0 -> forall t1 : term, P (vdef n t t0 :: Γ) t1 -> P Γ (gLetIn n t t0 t1)) ->
    (forall Γ (t u : term), P Γ t -> P Γ u -> P Γ (gApp t u)) ->
    (forall Γ (s : kername) (u : list Level.t), P Γ (gConst s u)) ->
    (forall Γ (i : inductive) (u : list Level.t), P Γ (gInd i u)) ->
    (forall Γ (i : inductive) (n : nat) (u : list Level.t), P Γ (gConstruct i n u)) ->
    (forall Γ (p : inductive * nat) (t : term),
        P Γ t -> forall t0 : term, P Γ t0 -> forall l : list (nat * term),
            tCaseBrsProp (P Γ) l -> P Γ (gCase p t t0 l)) ->
    (forall Γ (s : projection) (t : term), P Γ t -> P Γ (gProj s t)) ->
    (forall Γ (m : mfixpoint term) (n : nat), tFixProp (P Γ) (P (Γ ,,, fix_context m)) m -> P Γ (gFix m n)) ->
    (forall Γ (m : mfixpoint term) (n : nat), tFixProp (P Γ) (P (Γ ,,, fix_context m)) m -> P Γ (gCoFix m n)) ->
    (forall Γ A, P Γ A -> P Γ (gUkn A)) ->
    (forall Γ (A t : term), P Γ A -> P Γ t -> P Γ (gCast A t)) ->
...
Fixpoint subst_app (t : term) (us : list term) : term :=
  match t, us with
  | gLambda _ A t, u :: us => subst_app (t {0 := u}) us
  | _, [] => t
  | _, _ => mkApps t us
  end.
