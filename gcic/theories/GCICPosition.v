...
From MetaCoq.GCIC Require Import GCICAst GCICInduction GCICReflect GCICEquality GCICLiftSubst.
...
Fixpoint validpos t (p : position) {struct p} :=
  ...
  | c :: p =>
    match c, t with
    | app_l, gApp u v => validpos u p
    | app_r, gApp u v => validpos v p
    | case_p, gCase indn pr c brs => validpos pr p
    | case_c, gCase indn pr c brs => validpos c p
    | case_brs n, gCase indn pr c brs =>
        match nth_error brs n with
        | Some (_, br) => validpos br p
        | None => false
        end
    | proj_c, gProj pr c => validpos c p
    | fix_mfix_ty n, gFix mfix idx =>
        match nth_error mfix n with
        | Some d => validpos d.(dtype) p
        | None => false
        end
    | fix_mfix_bd n, gFix mfix idx =>
        match nth_error mfix n with
        | Some d => validpos d.(dbody) p
        | None => false
        end
    | lam_ty, gLambda na A t => validpos A p
    | lam_tm, gLambda na A t => validpos t p
    | prod_l, gProd na A B => validpos A p
    | prod_r, gProd na A B => validpos B p
    | let_bd, gLetIn na b B t => validpos b p
    | let_ty, gLetIn na b B t => validpos B p
    | let_in, gLetIn na b B t => validpos t p
    | ukn_ty, gUkn A => validpos A p
    | cast_ty, gCast A t => validpos A p
    | cast_tm, gCast A t => validpos t p
    | _, _ => false
    end
  end.
...
Definition dapp_l u v (p : pos u) : pos (gApp u v) :=
  exist (app_l :: proj1_sig p) (proj2_sig p).

Definition dapp_r u v (p : pos v) : pos (gApp u v) :=
  exist (app_r :: proj1_sig p) (proj2_sig p).

Definition dcase_p indn pr c brs (p : pos pr) : pos (gCase indn pr c brs) :=
  exist (case_p :: proj1_sig p) (proj2_sig p).

Definition dcase_c indn pr c brs (p : pos c) : pos (gCase indn pr c brs) :=
...
(* Equations dcase_brs (n : nat) (indn : inductive × nat)
  ...
  (p : pos br) : pos (gCase indn pr c brs) :=
...
Definition dproj_c pr c (p : pos c) : pos (gProj pr c) :=
  exist (proj_c :: proj1_sig p) (proj2_sig p).

Definition dlam_ty na A t (p : pos A) : pos (gLambda na A t) :=
  exist (lam_ty :: proj1_sig p) (proj2_sig p).

Definition dlam_tm na A t (p : pos t) : pos (gLambda na A t) :=
  exist (lam_tm :: proj1_sig p) (proj2_sig p).

Definition dprod_l na A B (p : pos A) : pos (gProd na A B) :=
  exist (prod_l :: proj1_sig p) (proj2_sig p).

Definition dprod_r na A B (p : pos B) : pos (gProd na A B) :=
  exist (prod_r :: proj1_sig p) (proj2_sig p).

Definition dlet_bd na b B t (p : pos b) : pos (gLetIn na b B t) :=
  exist (let_bd :: proj1_sig p) (proj2_sig p).

Definition dlet_ty na b B t (p : pos B) : pos (gLetIn na b B t) :=
  exist (let_ty :: proj1_sig p) (proj2_sig p).

Definition dlet_in na b B t (p : pos t) : pos (gLetIn na b B t) :=
  exist (let_in :: proj1_sig p) (proj2_sig p).

Definition dukn_ty A (p : pos A) : pos (gUkn A) :=
  exist (ukn_ty :: proj1_sig p) (proj2_sig p).

Definition dcast_ty A t (p : pos A) : pos (gCast A t) :=
  exist (cast_ty :: proj1_sig p) (proj2_sig p).

Definition dcast_tm A t(p : pos t) : pos (gCast A t) :=
...
Proof.
  ...
  assert (
    forall n indn pr c brs m br (p : pos br)
      ...
      (e1 : validpos (gCase indn pr c brs) (case_brs n :: proj1_sig p) = true),
      Acc posR p ->
  ...
  assert (
    forall n mfix idx d (p : pos d.(dtype))
      ...
      (e1 : validpos (gFix mfix idx) (fix_mfix_ty n :: proj1_sig p)),
      Acc posR p ->
  ...
  assert (
    forall n mfix idx d (p : pos d.(dbody))
      (e : nth_error mfix n = Some d)
      (e1 : validpos (gFix mfix idx) (fix_mfix_bd n :: proj1_sig p)),
      Acc posR p ->
  ...
  assert (forall A p, Acc posR p -> Acc posR (dukn_ty A p))
    as Acc_ukn_ty.
  { intros A p h.
    induction h as [p ih1 ih2].
    constructor. intros [q e] h.
  ...
  assert (forall A t p, Acc posR p -> Acc posR (dcast_ty A t p))
    as Acc_cast_ty.
  { intros A t p h.
    induction h as [p ih1 ih2].
    constructor. intros [q e] h.
  ...
  assert (forall A t p, Acc posR p -> Acc posR (dcast_tm A t p))
    as Acc_cast_tm.
  { intros A t p h.
    induction h as [p ih1 ih2].
    constructor. intros [q e] h.
  ...
  - destruct q as [q e]. destruct q as [| c q].
    + constructor. intros [p e'] h.
      ...
      dependent destruction h.
      destruct c ; noconf e'.
      eapply Acc_ukn_ty with (p := exist p0 e').
      eapply IHt.
    + destruct c ; noconf e.
      eapply Acc_ukn_ty with (p := exist q e).
      eapply IHt.
  - destruct q as [q e]. destruct q as [| c q].
    + constructor. intros [p e'] h.
      unfold posR in h. cbn in h.
      dependent destruction h.
      destruct c ; noconf e'.
      * eapply Acc_cast_ty with (p := exist p0 e').
        eauto.
      * eapply Acc_cast_tm with (p := exist p0 e').
        eauto.
    + destruct c ; noconf e.
      * eapply Acc_cast_ty with (p := exist q e).
        eauto.
...
Fixpoint atpos t (p : position) {struct p} : term :=
  ...
  | c :: p =>
    match c, t with
    | app_l, gApp u v => atpos u p
    | app_r, gApp u v => atpos v p
    | case_p, gCase indn pr c brs => atpos pr p
    | case_c, gCase indn pr c brs => atpos c p
    | case_brs n, gCase indn pr c brs =>
        match nth_error brs n with
        | Some (_, br) => atpos br p
        | None => gRel 0
        end
    | proj_c, gProj pr c => atpos c p
    | fix_mfix_ty n, gFix mfix idx =>
        match nth_error mfix n with
    ...
    | lam_ty, gLambda na A t => atpos A p
    | lam_tm, gLambda na A t => atpos t p
    | prod_l, gProd na A B => atpos A p
    | prod_r, gProd na A B => atpos B p
    | let_bd, gLetIn na b B t => atpos b p
    | let_ty, gLetIn na b B t => atpos B p
    | let_in, gLetIn na b B t => atpos t p
    | ukn_ty, gUkn A => atpos A p
    | cast_ty, gCast A t => atpos A p
    | cast_tm, gCast A t => atpos t p
    | _, _ => gRel 0
    end
  end.
...
Proof.
  assert (forall p, atpos (gRel 0) p = gRel 0) as hh.
  { intros p. destruct p.
    - reflexivity.
...
| Ukn_ty (π : stack)
| Cast_ty (t : term) (π : stack)
| Cast_tm (A : term) (π : stack).
...
Fixpoint zipc t stack :=
  match stack with
  | ε => t
  | App u π => zipc (gApp t u) π
  | Fix f n args π => zipc (gApp (mkApps (gFix f n) args) t) π
  | Fix_mfix_ty na bo ra mfix1 mfix2 idx π =>
      zipc (gFix (mfix1 ++ mkdef _ na t bo ra :: mfix2) idx) π
  | Fix_mfix_bd na ty ra mfix1 mfix2 idx π =>
      zipc (gFix (mfix1 ++ mkdef _ na ty t ra :: mfix2) idx) π
  | CoFix f n args π => zipc (gApp (mkApps (gCoFix f n) args) t) π
  | Case_p indn c brs π => zipc (gCase indn t c brs) π
  | Case indn pred brs π => zipc (gCase indn pred t brs) π
  | Case_brs indn pred c m brs1 brs2 π =>
      zipc (gCase indn pred c (brs1 ++ (m,t) :: brs2)) π
  | Proj p π => zipc (gProj p t) π
  | Prod_l na B π => zipc (gProd na t B) π
  | Prod_r na A π => zipc (gProd na A t) π
  | Lambda_ty na b π => zipc (gLambda na t b) π
  | Lambda_tm na A π => zipc (gLambda na A t) π
  | LetIn_bd na B u π => zipc (gLetIn na t B u) π
  | LetIn_ty na b u π => zipc (gLetIn na b t u) π
  | LetIn_in na b B π => zipc (gLetIn na b B t) π
  | coApp u π => zipc (gApp u t) π
  | Ukn_ty π => zipc (gUkn t) π
  | Cast_ty t' π => zipc (gCast t t') π
  | Cast_tm A π => zipc (gCast A t) π
  end.
...
Fixpoint stack_context π : context :=
  ...
  | Ukn_ty π => stack_context π
  | Cast_ty t π => stack_context π
  | Cast_tm A π => stack_context π
  end.
...
Fixpoint stack_position π : position :=
  ...
  | Ukn_ty ρ => stack_position ρ ++ [ukn_ty] 
  | Cast_ty t ρ => stack_position ρ ++ [cast_ty]
  | Cast_tm A ρ => stack_position ρ ++ [cast_tm]
  end.
...
Section Stacks.
  ...
  Fixpoint stack_cat (ρ θ : stack) : stack :=
    ...
    | Ukn_ty ρ => Ukn_ty (stack_cat ρ θ)
    | Cast_ty t ρ => Cast_ty t (stack_cat ρ θ)
    | Cast_tm A ρ => Cast_tm A (stack_cat ρ θ)
    end.
  ...
  Fixpoint rev_stack π :=
    ...
    | Ukn_ty ρ => rev_stack ρ +++ Ukn_ty ε
    | Cast_ty t ρ => rev_stack ρ +++ Cast_ty t ε
    | Cast_tm A ρ => rev_stack ρ +++ Cast_tm A ε
    end.
  ...
  Proof.
    intros P hε hApp hFix hFixty hFixbd hCoFix hCasep hCase hCasebrs hProj
      hProdl hProdr hLamty hLamtm hLetbd hLetty hLetin hcoApp hUknty hCastty hCasttm.
