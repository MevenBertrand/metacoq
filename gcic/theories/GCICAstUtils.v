...
From MetaCoq.GCIC Require Import GCICAst GCICSize.
...
Fixpoint string_of_term (t : term) :=
  match t with
  | gRel n => "Rel(" ++ string_of_nat n ++ ")"
  | gVar n => "Var(" ++ n ++ ")"
  | gEvar ev args => "Evar(" ++ string_of_nat ev ++ "," ++ string_of_list string_of_term args ++ ")"
  | gSort s => "Sort(" ++ string_of_sort s ++ ")"
  | gProd na b t => "Prod(" ++ string_of_name na ++ "," ++
                            string_of_term b ++ "," ++ string_of_term t ++ ")"
  | gLambda na b t => "Lambda(" ++ string_of_name na ++ "," ++ string_of_term b
                                ++ "," ++ string_of_term t ++ ")"
  | gLetIn na b t' t => "LetIn(" ++ string_of_name na ++ "," ++ string_of_term b
                                 ++ "," ++ string_of_term t' ++ "," ++ string_of_term t ++ ")"
  | gApp f l => "App(" ++ string_of_term f ++ "," ++ string_of_term l ++ ")"
  | gConst c u => "Const(" ++ string_of_kername c ++ "," ++ string_of_universe_instance u ++ ")"
  | gInd i u => "Ind(" ++ string_of_inductive i ++ "," ++ string_of_universe_instance u ++ ")"
  | gConstruct i n u => "Construct(" ++ string_of_inductive i ++ "," ++ string_of_nat n ++ ","
  ...
  | gProj (ind, i, k) c =>
    "Proj(" ++ string_of_inductive ind ++ "," ++ string_of_nat i ++ "," ++ string_of_nat k ++ ","
            ++ string_of_term c ++ ")"
  | gFix l n => "Fix(" ++ (string_of_list (string_of_def string_of_term) l) ++ "," ++ string_of_nat n ++ ")"
  | gCoFix l n => "CoFix(" ++ (string_of_list (string_of_def string_of_term) l) ++ "," ++ string_of_nat n ++ ")"
  | gUkn A => "Ukn("++ string_of_term A ++ ")"
  | gCast A t => "Cast("++ string_of_term A ++ "," ++ string_of_term t ++ ")"
  end.
Local Close Scope string_scope.

Fixpoint decompose_app_rec (t : term) l :=
  match t with
  | gApp f a => decompose_app_rec f (a :: l)
  | _ => (t, l)
  end.
...
Fixpoint decompose_prod (t : term) : (list name) * (list term) * term :=
  match t with
  | gProd n A B => let (nAs, B) := decompose_prod B in
                  let (ns, As) := nAs in
                  (n :: ns, A :: As, B)
  | _ => ([], [], t)
  end.
...
Fixpoint decompose_prod_assum (Γ : context) (t : term) : context * term :=
  match t with
  | gProd n A B => decompose_prod_assum (Γ ,, vass n A) B
  | gLetIn na b bty b' => decompose_prod_assum (Γ ,, vdef na b bty) b'
  | _ => (Γ, t)
  end.
...
Fixpoint decompose_prod_n_assum (Γ : context) n (t : term) : option (context * term) :=
  ...
  | S n =>
    match t with
    | gProd na A B => decompose_prod_n_assum (Γ ,, vass na A) n B
    | gLetIn na b bty b' => decompose_prod_n_assum (Γ ,, vdef na b bty) n b'
    | _ => None
    end
  end.
...
Lemma is_ind_app_head_mkApps ind u l : is_ind_app_head (mkApps (gInd ind u) l).
Proof.
...
Lemma reln_list_lift_above l p Γ :
  Forall (fun x => exists n, x = gRel n /\ p <= n /\ n < p + length Γ) l ->
  Forall (fun x => exists n, x = gRel n /\ p <= n /\ n < p + length Γ) (reln l p Γ).
Proof.
  ...
  simpl in *.
  specialize (IHΓ (gRel p :: l) (S p) p0 ltac:(lia)). rewrite <- Nat.add_succ_comm, Nat.add_1_r.
...
Lemma to_extended_list_k_spec Γ k :
  Forall (fun x => exists n, x = gRel n /\ k <= n /\ n < k + length Γ) (to_extended_list_k Γ k).
Proof.
...
Lemma to_extended_list_lift_above Γ :
  Forall (fun x => exists n, x = gRel n /\ n < length Γ) (to_extended_list Γ).
Proof.
...
Fixpoint reln_alt p Γ :=
  ...
  | {| decl_body := None |} :: Γ => gRel p :: reln_alt (p + 1) Γ
  end.
...
Lemma to_extended_list_k_cons d Γ k :
  ...
  | None => to_extended_list_k Γ (S k) ++ [gRel k]
...
Lemma mkApps_Fix_spec mfix idx args t : mkApps (gFix mfix idx) args = t ->
                                        match decompose_app t with
                                        | (gFix mfix idx, args') => args' = args
                                        | _ => False
                                        end.
Proof.
...
Lemma decompose_app_rec_tFix mfix idx args t l :
  decompose_app_rec t l = (gFix mfix idx, args) -> mkApps t l = mkApps (gFix mfix idx) args.
Proof.
...
Lemma decompose_app_tFix mfix idx args t :
  decompose_app t = (gFix mfix idx, args) -> t = mkApps (gFix mfix idx) args.
...
Ltac prepare_discr :=
  repeat match goal with
         | [ H : mkApps ?f ?l = gApp ?y ?r |- _ ] => change (mkApps f l = mkApps y [r]) in H
         | [ H : gApp ?f ?l = mkApps ?y ?r |- _ ] => change (mkApps f [l] = mkApps y r) in H
         | [ H : mkApps ?x ?l = ?y |- _ ] =>
           match y with
           | mkApps _ _ => fail 1
...
Proof.
  ...
  now rewrite [gApp _ _](mkApps_nested hd _ [f2]).
