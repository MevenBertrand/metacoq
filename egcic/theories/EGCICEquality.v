...
| eq_Rel n  :
    eq_term_upto_univ Re Rle (eRel n) (eRel n)

| eq_Evar e args args' :
    All2 (eq_term_upto_univ Re Re) args args' ->
    eq_term_upto_univ Re Rle (eEvar e args) (eEvar e args')

| eq_Var id :
    eq_term_upto_univ Re Rle (eVar id) (eVar id)

| eq_Sort s s' :
    Rle s s' ->
    eq_term_upto_univ Re Rle (eSort s) (eSort s')

| eq_App t t' u u' :
    eq_term_upto_univ Re Rle t t' ->
    eq_term_upto_univ Re Re u u' ->
    eq_term_upto_univ Re Rle (eApp t u) (eApp t' u')

| eq_Const c u u' :
    R_universe_instance Re u u' ->
    eq_term_upto_univ Re Rle (eConst c u) (eConst c u')

| eq_Ind i u u' :
    R_universe_instance Re u u' ->
    eq_term_upto_univ Re Rle (eInd i u) (eInd i u')

| eq_Construct i k u u' :
    R_universe_instance Re u u' ->
    eq_term_upto_univ Re Rle (eConstruct i k u) (eConstruct i k u')

| eq_Lambda na na' ty ty' t t' :
    eq_term_upto_univ Re Re ty ty' ->
    eq_term_upto_univ Re Rle t t' ->
    eq_term_upto_univ Re Rle (eLambda na ty t) (eLambda na' ty' t')

| eq_Prod na na' a a' b b' :
    eq_term_upto_univ Re Re a a' ->
    eq_term_upto_univ Re Rle b b' ->
    eq_term_upto_univ Re Rle (eProd na a b) (eProd na' a' b')

| eq_LetIn na na' t t' ty ty' u u' :
    eq_term_upto_univ Re Re t t' ->
    eq_term_upto_univ Re Re ty ty' ->
    eq_term_upto_univ Re Rle u u' ->
    eq_term_upto_univ Re Rle (eLetIn na t ty u) (eLetIn na' t' ty' u')
...
| eq_Case indn p p' c c' brs brs' :
    ...
    eq_term_upto_univ Re Rle (eCase indn p c brs) (eCase indn p' c' brs')

| eq_Proj p c c' :
    eq_term_upto_univ Re Re c c' ->
    eq_term_upto_univ Re Rle (eProj p c) (eProj p c')
...
| eq_Fix mfix mfix' idx :
    ...
    eq_term_upto_univ Re Rle (eFix mfix idx) (eFix mfix' idx)
...
| eq_CoFix mfix mfix' idx :
    ...
    eq_term_upto_univ Re Rle (eCoFix mfix idx) (eCoFix mfix' idx)

| eq_Err e A A' :
    eq_term_upto_univ Re Re A A' ->
    eq_term_upto_univ Re Rle (eErr e A) (eErr e A')

| eq_Cast A A' B B' t t' :
    eq_term_upto_univ Re Rle A A' ->
    eq_term_upto_univ Re Re B B' ->
    eq_term_upto_univ Re Re t t' ->
    eq_term_upto_univ Re Rle (eCast A B t) (eCast A' B' t').
...
Fixpoint eqb_term_upto_univ (equ lequ : Universe.t -> Universe.t -> bool) (u v : term) : bool :=
  ...
  | eErr e A, eErr e' A' =>
    eqb e e' && eqb_term_upto_univ equ equ A A'
...
Local Open Scope egcic.

Definition upto_names := eq_term_upto_univ eq eq.

Infix "≡" := upto_names (at level 60) : egcic.

Infix "≡'" := (eq_term_upto_univ eq eq) (at level 60) : egcic.
