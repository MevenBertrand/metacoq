...
From MetaCoq.EGCIC Require Import EGCICAst EGCICInduction EGCICReflect EGCICEquality EGCICLiftSubst.
...
Fixpoint validpos t (p : position) {struct p} :=
  ...
  | c :: p =>
    match c, t with
    | app_l, eApp u v => validpos u p
    | app_r, eApp u v => validpos v p
    | case_p, eCase indn pr c brs => validpos pr p
    | case_c, eCase indn pr c brs => validpos c p
    | case_brs n, eCase indn pr c brs =>
        match nth_error brs n with
        | Some (_, br) => validpos br p
        | None => false
        end
    | proj_c, eProj pr c => validpos c p
    | fix_mfix_ty n, eFix mfix idx =>
        match nth_error mfix n with
        | Some d => validpos d.(dtype) p
        | None => false
        end
    | fix_mfix_bd n, eFix mfix idx =>
        match nth_error mfix n with
        | Some d => validpos d.(dbody) p
        | None => false
        end
    | lam_ty, eLambda na A t => validpos A p
    | lam_tm, eLambda na A t => validpos t p
    | prod_l, eProd na A B => validpos A p
    | prod_r, eProd na A B => validpos B p
    | let_bd, eLetIn na b B t => validpos b p
    | let_ty, eLetIn na b B t => validpos B p
    | let_in, eLetIn na b B t => validpos t p
    | err_ty, eErr e A => validpos A p
    | cast_dom, eCast A B t => validpos A p
    | cast_cod, eCast A B t => validpos B p
    | cast_tm, eCast A B t => validpos t p
    | _, _ => false
    end
  end.
...
Definition dapp_l u v (p : pos u) : pos (eApp u v) :=
  exist (app_l :: proj1_sig p) (proj2_sig p).

Definition dapp_r u v (p : pos v) : pos (eApp u v) :=
  exist (app_r :: proj1_sig p) (proj2_sig p).

Definition dcase_p indn pr c brs (p : pos pr) : pos (eCase indn pr c brs) :=
  exist (case_p :: proj1_sig p) (proj2_sig p).

Definition dcase_c indn pr c brs (p : pos c) : pos (eCase indn pr c brs) :=
...
(* Equations dcase_brs (n : nat) (indn : inductive × nat)
  ...
  (p : pos br) : pos (eCase indn pr c brs) :=
  dcase_brs n indn pr c brs m br h p :=
    exist (case_brs n :: proj1_sig p) _.
Next Obligation.
  rewrite h. exact (proj2_sig p).
Qed.
Transparent dcase_brs. *)

Definition dproj_c pr c (p : pos c) : pos (eProj pr c) :=
  exist (proj_c :: proj1_sig p) (proj2_sig p).

Definition dlam_ty na A t (p : pos A) : pos (eLambda na A t) :=
  exist (lam_ty :: proj1_sig p) (proj2_sig p).

Definition dlam_tm na A t (p : pos t) : pos (eLambda na A t) :=
  exist (lam_tm :: proj1_sig p) (proj2_sig p).

Definition dprod_l na A B (p : pos A) : pos (eProd na A B) :=
  exist (prod_l :: proj1_sig p) (proj2_sig p).

Definition dprod_r na A B (p : pos B) : pos (eProd na A B) :=
  exist (prod_r :: proj1_sig p) (proj2_sig p).

Definition dlet_bd na b B t (p : pos b) : pos (eLetIn na b B t) :=
  exist (let_bd :: proj1_sig p) (proj2_sig p).

Definition dlet_ty na b B t (p : pos B) : pos (eLetIn na b B t) :=
  exist (let_ty :: proj1_sig p) (proj2_sig p).

Definition dlet_in na b B t (p : pos t) : pos (eLetIn na b B t) :=
  exist (let_in :: proj1_sig p) (proj2_sig p).

Definition derr_ty e A (p : pos A) : pos (eErr e A) :=
  exist (err_ty :: proj1_sig p) (proj2_sig p).

Definition dcast_dom A B t (p : pos A) : pos (eCast A B t) :=
  exist (cast_dom :: proj1_sig p) (proj2_sig p).

Definition dcast_cod A B t (p : pos B) : pos (eCast A B t) :=
  exist (cast_cod :: proj1_sig p) (proj2_sig p).

Definition dcast_tm A B t (p : pos t) : pos (eCast A B t) :=
exist (cast_tm :: proj1_sig p) (proj2_sig p).
...
Proof.
  ...
  assert (
    forall n indn pr c brs m br (p : pos br)
      ...
      (e1 : validpos (eCase indn pr c brs) (case_brs n :: proj1_sig p) = true),
      Acc posR p ->
  ...
  assert (
    forall n mfix idx d (p : pos d.(dtype))
      ...
      (e1 : validpos (eFix mfix idx) (fix_mfix_ty n :: proj1_sig p)),
      Acc posR p ->
  ...
  assert (
    forall n mfix idx d (p : pos d.(dbody))
      (e : nth_error mfix n = Some d)
      (e1 : validpos (eFix mfix idx) (fix_mfix_bd n :: proj1_sig p)),
      Acc posR p ->
  ...
  assert (forall e A p, Acc posR p -> Acc posR (derr_ty e A p))
    as Acc_err_ty.
  { intros e A p h.
    induction h as [p ih1 ih2].
    constructor. intros [q eq] h.
    dependent destruction h. cbn in e.
    eapply (ih2 (exist p0 eq)). assumption.
  }
  assert (forall A B t p, Acc posR p -> Acc posR (dcast_dom A B t p))
    as Acc_cast_dom.
  { intros A B t p h.
    induction h as [p ih1 ih2].
    constructor. intros [q e] h.
  ...
  assert (forall A B t p, Acc posR p -> Acc posR (dcast_cod A B t p))
    as Acc_cast_cod.
  { intros A B t p h.
    induction h as [p ih1 ih2].
    constructor. intros [q e] h.
  ...
  assert (forall A B t p, Acc posR p -> Acc posR (dcast_tm A B t p))
    as Acc_cast_tm.
  { intros A B t p h.
    induction h as [p ih1 ih2].
    constructor. intros [q e] h.
  ...
  - destruct q as [q e]. destruct q as [| c q].
    ...
    + destruct c. all: noconf e.
      ...
      * simpl in e.
        ...
        -- simpl. rewrite e1 in e. assumption.
        -- eapply ihm.
  - destruct q as [q eq]. destruct q as [|c q].
    + constructor. intros [p eq'] h.
      unfold posR in h. cbn in h.
      dependent destruction h.
      destruct c ; noconf eq'.
      eapply Acc_err_ty with (p := exist p0 eq').
      eapply IHt.
    + destruct c ; noconf eq.
      eapply Acc_err_ty with (p := exist q eq).
      eapply IHt.  
  - destruct q as [q e]. destruct q as [| c q].
    + constructor. intros [p e'] h.
      unfold posR in h. cbn in h.
      dependent destruction h.
      destruct c ; noconf e'.
      * eapply Acc_cast_dom with (p := exist p0 e').
        eauto.
      * eapply Acc_cast_cod with (p := exist p0 e').
        eauto.
      * eapply Acc_cast_tm with (p := exist p0 e').
        eauto.
    + destruct c ; noconf e.
      * eapply Acc_cast_dom with (p := exist q e).
        eauto.
      * eapply Acc_cast_cod with (p := exist q e).
        eauto.
      * eapply Acc_cast_tm with (p := exist q e).
        eauto.
Qed.
...
Fixpoint atpos t (p : position) {struct p} : term :=
  ...
  | c :: p =>
    match c, t with
    | app_l, eApp u v => atpos u p
    | app_r, eApp u v => atpos v p
    | case_p, eCase indn pr c brs => atpos pr p
    | case_c, eCase indn pr c brs => atpos c p
    | case_brs n, eCase indn pr c brs =>
        match nth_error brs n with
        | Some (_, br) => atpos br p
        | None => eRel 0
        end
    | proj_c, eProj pr c => atpos c p
    | fix_mfix_ty n, eFix mfix idx =>
        match nth_error mfix n with
    ...
    | lam_ty, eLambda na A t => atpos A p
    | lam_tm, eLambda na A t => atpos t p
    | prod_l, eProd na A B => atpos A p
    | prod_r, eProd na A B => atpos B p
    | let_bd, eLetIn na b B t => atpos b p
    | let_ty, eLetIn na b B t => atpos B p
    | let_in, eLetIn na b B t => atpos t p
    | err_ty, eErr e A => atpos A p
    | cast_dom, eCast A B t => atpos A p
    | cast_cod, eCast A B t => atpos B p
    | cast_tm, eCast A B t => atpos t p
    | _, _ => eRel 0
    end
  end.
...
Proof.
  assert (forall p, atpos (eRel 0) p = eRel 0) as hh.
  { intros p. destruct p.
    - reflexivity.
...
| Err_ty (e : err_type) (π : stack)
| Cast_dom (B t : term) (π : stack)
| Cast_cod (A t: term) (π : stack)
| Cast_tm (A B : term) (π : stack).
...
Fixpoint zipc t stack :=
  match stack with
  | ε => t
  | App u π => zipc (eApp t u) π
  | Fix f n args π => zipc (eApp (mkApps (eFix f n) args) t) π
  | Fix_mfix_ty na bo ra mfix1 mfix2 idx π =>
      zipc (eFix (mfix1 ++ mkdef _ na t bo ra :: mfix2) idx) π
  | Fix_mfix_bd na ty ra mfix1 mfix2 idx π =>
      zipc (eFix (mfix1 ++ mkdef _ na ty t ra :: mfix2) idx) π
  | CoFix f n args π => zipc (eApp (mkApps (eCoFix f n) args) t) π
  | Case_p indn c brs π => zipc (eCase indn t c brs) π
  | Case indn pred brs π => zipc (eCase indn pred t brs) π
  | Case_brs indn pred c m brs1 brs2 π =>
      zipc (eCase indn pred c (brs1 ++ (m,t) :: brs2)) π
  | Proj p π => zipc (eProj p t) π
  | Prod_l na B π => zipc (eProd na t B) π
  | Prod_r na A π => zipc (eProd na A t) π
  | Lambda_ty na b π => zipc (eLambda na t b) π
  | Lambda_tm na A π => zipc (eLambda na A t) π
  | LetIn_bd na B u π => zipc (eLetIn na t B u) π
  | LetIn_ty na b u π => zipc (eLetIn na b t u) π
  | LetIn_in na b B π => zipc (eLetIn na b B t) π
  | coApp u π => zipc (eApp u t) π
  | Err_ty e π => zipc (eErr e t) π
  | Cast_dom B t' π => zipc (eCast t B t') π
  | Cast_cod A t' π => zipc (eCast A t t') π
  | Cast_tm A B π => zipc (eCast A B t) π
  end.
...
Fixpoint stack_context π : context :=
  ...
  | Err_ty e π => stack_context π
  | Cast_dom B t π => stack_context π
  | Cast_cod A t π => stack_context π
  | Cast_tm A B π => stack_context π
  end.
...
Fixpoint stack_position π : position :=
  ...
  | Err_ty e ρ => stack_position ρ ++ [err_ty]
  | Cast_dom B t ρ => stack_position ρ ++ [cast_dom]
  | Cast_cod A t ρ => stack_position ρ ++ [cast_cod]
  | Cast_tm A B ρ => stack_position ρ ++ [cast_tm]
  end.
...
Section Stacks.
  ...
  Proof.
    ...
    induction π ; intros u' v eq.
    all: try solve [ cbn in eq ; apply IHπ in eq ; inversion eq ; reflexivity ].
  ...
  Fixpoint stack_cat (ρ θ : stack) : stack :=
    ...
    | Err_ty e ρ => Err_ty e (stack_cat ρ θ)
    | Cast_dom B t ρ => Cast_dom B t (stack_cat ρ θ)
    | Cast_cod A t ρ => Cast_cod A t (stack_cat ρ θ)
    | Cast_tm A B ρ => Cast_tm A B (stack_cat ρ θ)
    end.
  ...
  Fixpoint rev_stack π :=
    ...
    | Err_ty e ρ => rev_stack ρ +++ Err_ty e ε
    | Cast_dom B t ρ => rev_stack ρ +++ Cast_dom B t ε
    | Cast_cod A t ρ => rev_stack ρ +++ Cast_cod A t ε
    | Cast_tm A B ρ => rev_stack ρ +++ Cast_tm A B ε
    end.
  ...
  Proof.
    intros P hε hApp hFix hFixty hFixbd hCoFix hCasep hCase hCasebrs hProj
      hProdl hProdr hLamty hLamtm hLetbd hLetty hLetin hcoApp hErrty hCastdom hCastcod hCasttm.
