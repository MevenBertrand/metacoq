...
From MetaCoq.EGCIC Require Import EGCICAst EGCICSize.
...
Definition string_of_errtype (e : err_type) :=
  match e with
  | ukn => "ukn"
  | rai => "rai"
  end.
...
Fixpoint string_of_term (t : term) :=
  match t with
  | eRel n => "Rel(" ++ string_of_nat n ++ ")"
  | eVar n => "Var(" ++ n ++ ")"
  | eEvar ev args => "Evar(" ++ string_of_nat ev ++ "," ++ string_of_list string_of_term args ++ ")"
  | eSort s => "Sort(" ++ string_of_sort s ++ ")"
  | eProd na b t => "Prod(" ++ string_of_name na ++ "," ++
                            string_of_term b ++ "," ++ string_of_term t ++ ")"
  | eLambda na b t => "Lambda(" ++ string_of_name na ++ "," ++ string_of_term b
                                ++ "," ++ string_of_term t ++ ")"
  | eLetIn na b t' t => "LetIn(" ++ string_of_name na ++ "," ++ string_of_term b
                                 ++ "," ++ string_of_term t' ++ "," ++ string_of_term t ++ ")"
  | eApp f l => "App(" ++ string_of_term f ++ "," ++ string_of_term l ++ ")"
  | eConst c u => "Const(" ++ string_of_kername c ++ "," ++ string_of_universe_instance u ++ ")"
  | eInd i u => "Ind(" ++ string_of_inductive i ++ "," ++ string_of_universe_instance u ++ ")"
  | eConstruct i n u => "Construct(" ++ string_of_inductive i ++ "," ++ string_of_nat n ++ ","
  ...
  | eProj (ind, i, k) c =>
    "Proj(" ++ string_of_inductive ind ++ "," ++ string_of_nat i ++ "," ++ string_of_nat k ++ ","
            ++ string_of_term c ++ ")"
  | eFix l n => "Fix(" ++ (string_of_list (string_of_def string_of_term) l) ++ "," ++ string_of_nat n ++ ")"
  | eCoFix l n => "CoFix(" ++ (string_of_list (string_of_def string_of_term) l) ++ "," ++ string_of_nat n ++ ")"
  | eErr e A => "Err("++ (string_of_errtype e) ++
      "," ++ (string_of_term A) ++ ")"
  | eCast A B t => "Cast("++ string_of_term A ++ "," ++ string_of_term B ++ "," ++ string_of_term t ++ ")"
  end.
Local Close Scope string_scope.

Fixpoint decompose_app_rec (t : term) l :=
  match t with
  | eApp f a => decompose_app_rec f (a :: l)
  | _ => (t, l)
  end.
...
Fixpoint decompose_prod (t : term) : (list name) * (list term) * term :=
  match t with
  | eProd n A B => let (nAs, B) := decompose_prod B in
                  let (ns, As) := nAs in
                  (n :: ns, A :: As, B)
  | _ => ([], [], t)
  end.
...
Fixpoint decompose_prod_assum (Γ : context) (t : term) : context * term :=
  match t with
  | eProd n A B => decompose_prod_assum (Γ ,, vass n A) B
  | eLetIn na b bty b' => decompose_prod_assum (Γ ,, vdef na b bty) b'
  | _ => (Γ, t)
  end.
...
Fixpoint decompose_prod_n_assum (Γ : context) n (t : term) : option (context * term) :=
  ...
  | S n =>
    match t with
    | eProd na A B => decompose_prod_n_assum (Γ ,, vass na A) n B
    | eLetIn na b bty b' => decompose_prod_n_assum (Γ ,, vdef na b bty) n b'
    | _ => None
    end
  end.
...
Lemma is_ind_app_head_mkApps ind u l : is_ind_app_head (mkApps (eInd ind u) l).
Proof.
...
Lemma reln_list_lift_above l p Γ :
  Forall (fun x => exists n, x = eRel n /\ p <= n /\ n < p + length Γ) l ->
  Forall (fun x => exists n, x = eRel n /\ p <= n /\ n < p + length Γ) (reln l p Γ).
Proof.
  ...
  simpl in *.
  specialize (IHΓ (eRel p :: l) (S p) p0 ltac:(lia)). rewrite <- Nat.add_succ_comm, Nat.add_1_r.
...
Lemma to_extended_list_k_spec Γ k :
  Forall (fun x => exists n, x = eRel n /\ k <= n /\ n < k + length Γ) (to_extended_list_k Γ k).
Proof.
...
Lemma to_extended_list_lift_above Γ :
  Forall (fun x => exists n, x = eRel n /\ n < length Γ) (to_extended_list Γ).
Proof.
...
Fixpoint reln_alt p Γ :=
  ...
  | {| decl_body := None |} :: Γ => eRel p :: reln_alt (p + 1) Γ
  end.
...
Lemma to_extended_list_k_cons d Γ k :
  ...
  | None => to_extended_list_k Γ (S k) ++ [eRel k]
...
Lemma mkApps_Fix_spec mfix idx args t : mkApps (eFix mfix idx) args = t ->
                                        match decompose_app t with
                                        | (eFix mfix idx, args') => args' = args
                                        | _ => False
                                        end.
Proof.
...
Lemma decompose_app_rec_tFix mfix idx args t l :
  decompose_app_rec t l = (eFix mfix idx, args) -> mkApps t l = mkApps (eFix mfix idx) args.
Proof.
...
Lemma decompose_app_tFix mfix idx args t :
  decompose_app t = (eFix mfix idx, args) -> t = mkApps (eFix mfix idx) args.
...
Ltac prepare_discr :=
  repeat match goal with
         | [ H : mkApps ?f ?l = eApp ?y ?r |- _ ] => change (mkApps f l = mkApps y [r]) in H
         | [ H : eApp ?f ?l = mkApps ?y ?r |- _ ] => change (mkApps f [l] = mkApps y r) in H
         | [ H : mkApps ?x ?l = ?y |- _ ] =>
           match y with
           | mkApps _ _ => fail 1
...
Proof.
  ...
  now rewrite [eApp _ _](mkApps_nested hd _ [f2]).
