...
From MetaCoq.EGCIC Require Import EGCICAst EGCICAstUtils EGCICInduction.
...
Fixpoint rename f t : term :=
  match t with
  | eRel i => eRel (f i)
  | eEvar ev args => eEvar ev (List.map (rename f) args)
  | eLambda na T M => eLambda na (rename f T) (rename (shiftn 1 f) M)
  | eApp u v => eApp (rename f u) (rename f v)
  | eProd na A B => eProd na (rename f A) (rename (shiftn 1 f) B)
  | eLetIn na b t b' => eLetIn na (rename f b) (rename f t) (rename (shiftn 1 f) b')
  | eCase ind p c brs =>
    let brs' := List.map (on_snd (rename f)) brs in
    eCase ind (rename f p) (rename f c) brs'
  | eProj p c => eProj p (rename f c)
  | eFix mfix idx =>
  ...
  | eCoFix mfix idx =>
    let mfix' := List.map (map_def (rename f) (rename (shiftn (List.length mfix) f))) mfix in
    eCoFix mfix' idx
  | eErr  e A => eErr e (rename f A)
  | eCast A B t => eCast (rename f A) (rename f B) (rename f t)
  | x => x
  end.

Fixpoint lift n k t : term :=
  match t with
  | eRel i => eRel (if Nat.leb k i then (n + i) else i)
  | eEvar ev args => eEvar ev (List.map (lift n k) args)
  | eLambda na T M => eLambda na (lift n k T) (lift n (S k) M)
  | eApp u v => eApp (lift n k u) (lift n k v)
  | eProd na A B => eProd na (lift n k A) (lift n (S k) B)
  | eLetIn na b t b' => eLetIn na (lift n k b) (lift n k t) (lift n (S k) b')
  | eCase ind p c brs =>
    let brs' := List.map (on_snd (lift n k)) brs in
    eCase ind (lift n k p) (lift n k c) brs'
  | eProj p c => eProj p (lift n k c)
  | eFix mfix idx =>
  ...
  | eCoFix mfix idx =>
    ...
    let mfix' := List.map (map_def (lift n k) (lift n k')) mfix in
    eCoFix mfix' idx
  | eErr e A => eErr e (lift n k A)
  | eCast A B t => eCast (lift n k A) (lift n k B) (lift n k t)
  | x => x
  end.
...
Fixpoint subst s k u :=
  ...
  | eRel n =>
    if Nat.leb k n then
      ...
      | Some b => lift0 k b
      | None => eRel (n - List.length s)
      end
    else eRel n
  | eEvar ev args => eEvar ev (List.map (subst s k) args)
  | eLambda na T M => eLambda na (subst s k T) (subst s (S k) M)
  | eApp u v => eApp (subst s k u) (subst s k v)
  | eProd na A B => eProd na (subst s k A) (subst s (S k) B)
  | eLetIn na b ty b' => eLetIn na (subst s k b) (subst s k ty) (subst s (S k) b')
  | eCase ind p c brs =>
    let brs' := List.map (on_snd (subst s k)) brs in
    eCase ind (subst s k p) (subst s k c) brs'
  | eProj p c => eProj p (subst s k c)
  | eFix mfix idx =>
    let k' := List.length mfix + k in
  ...
  | eCoFix mfix idx =>
    ...
    let mfix' := List.map (map_def (subst s k) (subst s k')) mfix in
    eCoFix mfix' idx
  | eErr e A => eErr e (subst s k A)
  | eCast A B t => eCast (subst s k A) (subst s k B) (subst s k t)
  | x => x
  end.
...
Fixpoint closedn k (t : term) : bool :=
  match t with
  | eRel i => Nat.ltb i k
  | eEvar ev args => List.forallb (closedn k) args
  | eLambda _ T M | eProd _ T M => closedn k T && closedn (S k) M
  | eApp u v => closedn k u && closedn k v
  | eLetIn na b t b' => closedn k b && closedn k t && closedn (S k) b'
  | eCase ind p c brs =>
    let brs' := List.forallb (test_snd (closedn k)) brs in
    closedn k p && closedn k c && brs'
  | eProj p c => closedn k c
  | eFix mfix idx =>
    let k' := List.length mfix + k in
  ...
  | eCoFix mfix idx =>
    let k' := List.length mfix + k in
    List.forallb (test_def (closedn k) (closedn k')) mfix
  | eErr e A => closedn k A
  | eCast A B t => closedn k A && closedn k B && closedn k t
  | x => true
  end.
...
Lemma lift_rel_ge :
  forall k n p, p <= n -> lift k p (eRel n) = eRel (k + n).
Proof.
  intros; simpl in |- *.
  now elim (leb_spec p n).
Qed.

Lemma lift_rel_lt : forall k n p, p > n -> lift k p (eRel n) = eRel n.
Proof.
  intros; simpl in |- *.
  now elim (leb_spec p n).
Qed.

Lemma subst_rel_lt : forall u n k, k > n -> subst u k (eRel n) = eRel n.
Proof.
  simpl in |- *; intros.
...
Lemma subst_rel_gt :
  forall u n k, n >= k + length u -> subst u k (eRel n) = eRel (n - length u).
Proof.
  simpl in |- *; intros.
...
Proof.
  ...
  simpl. rewrite (IHl n k (eApp t a)). reflexivity.
Qed.
...
Fixpoint inst s u :=
  match u with
  | eRel n => s n
  | eEvar ev args => eEvar ev (List.map (inst s) args)
  | eLambda na T M => eLambda na (inst s T) (inst (up 1 s) M)
  | eApp u v => eApp (inst s u) (inst s v)
  | eProd na A B => eProd na (inst s A) (inst (up 1 s) B)
  | eLetIn na b ty b' => eLetIn na (inst s b) (inst s ty) (inst (up 1 s) b')
  | eCase ind p c brs =>
    let brs' := List.map (on_snd (inst s)) brs in
    eCase ind (inst s p) (inst s c) brs'
  | eProj p c => eProj p (inst s c)
  | eFix mfix idx =>
  ...
  | eCoFix mfix idx =>
    let mfix' := map (map_def (inst s) (inst (up (List.length mfix) s))) mfix in
    eCoFix mfix' idx
  | eErr e A => eErr e (inst s A)
  | eCast A B t => eCast (inst s A) (inst s B) (inst s t)
  | x => x
  end.
...
Definition shift : nat -> term := eRel ∘ S.
...
Definition Up σ : substitution := eRel 0 ⋅ (σ ∘s ↑).
...
Definition shiftk (k : nat) (x : nat) := eRel (k + x).
...
Lemma idsn_lt {n i} : i < n -> nth_error (idsn n) i = Some (eRel i).
Proof.
...
Lemma nth_error_idsn_Some :
  forall n k,
    k < n ->
    nth_error (idsn n) k = Some (eRel k).
Proof.
  intros n k h.
  induction n in k, h |- *.
  - inversion h.
...
Lemma inst_app {s t σ} : (eApp s t).[σ] = eApp s.[σ] t.[σ].
Proof. reflexivity. Qed.

Lemma inst_lam {na t b σ} : (eLambda na t b).[σ] = eLambda na t.[σ] b.[⇑ σ].
Proof.
  simpl. now rewrite up_Up.
Qed.

Lemma inst_prod {na t b σ} : (eProd na t b).[σ] = eProd na t.[σ] b.[⇑ σ].
Proof.
  simpl. now rewrite up_Up.
Qed.

Lemma inst_letin {na t b b' σ} : (eLetIn na t b b').[σ] = eLetIn na t.[σ] b.[σ] b'.[⇑ σ].
Proof.
  simpl. now rewrite up_Up.
Qed.

Lemma inst_fix {mfix idx σ} : (eFix mfix idx).[σ] =
                              eFix (map (map_def (inst σ) (inst (⇑^#|mfix| σ))) mfix) idx.
Proof.
  simpl. f_equal. apply map_ext. intros x. apply map_def_eq_spec. reflexivity.
  now rewrite up_Upn.
Qed.

Lemma inst_cofix {mfix idx σ} : (eCoFix mfix idx).[σ] =
                                eCoFix (map (map_def (inst σ) (inst (⇑^#|mfix| σ))) mfix) idx.
Proof.
  simpl. f_equal. apply map_ext. intros x. apply map_def_eq_spec. reflexivity.
  now rewrite up_Upn.
Qed.
...
Lemma subst_cons_0 t σ : (eRel 0).[t ⋅ σ] = t. Proof. reflexivity. Qed.
...
Lemma subst_cons_ren i f : (eRel i ⋅ ren f) =1 ren (subst_cons_gen i f).
Proof.
...
Lemma subst_cons_0_shift : eRel 0 ⋅ ↑ =1 ids.
...
Lemma term_forall_ctx_list_ind :
  forall P : context -> term -> Type,
    (forall Γ (n : nat), P Γ (eRel n)) ->
    (forall Γ (i : ident), P Γ (eVar i)) ->
    (forall Γ (n : nat) (l : list term), All (P Γ) l -> P Γ (eEvar n l)) ->
    (forall Γ s, P Γ (eSort s)) ->
    (forall Γ (n : name) (t : term), P Γ t -> forall t0 : term, P (vass n t :: Γ) t0 -> P Γ (eProd n t t0)) ->
    (forall Γ (n : name) (t : term), P Γ t -> forall t0 : term, P (vass n t :: Γ) t0 -> P Γ (eLambda n t t0)) ->
    (forall Γ (n : name) (t : term),
        P Γ t -> forall t0 : term, P Γ t0 -> forall t1 : term, P (vdef n t t0 :: Γ) t1 -> P Γ (eLetIn n t t0 t1)) ->
    (forall Γ (t u : term), P Γ t -> P Γ u -> P Γ (eApp t u)) ->
    (forall Γ (s : kername) (u : list Level.t), P Γ (eConst s u)) ->
    (forall Γ (i : inductive) (u : list Level.t), P Γ (eInd i u)) ->
    (forall Γ (i : inductive) (n : nat) (u : list Level.t), P Γ (eConstruct i n u)) ->
    (forall Γ (p : inductive * nat) (t : term),
        P Γ t -> forall t0 : term, P Γ t0 -> forall l : list (nat * term),
            tCaseBrsProp (P Γ) l -> P Γ (eCase p t t0 l)) ->
    (forall Γ (s : projection) (t : term), P Γ t -> P Γ (eProj s t)) ->
    (forall Γ (m : mfixpoint term) (n : nat), tFixProp (P Γ) (P (Γ ,,, fix_context m)) m -> P Γ (eFix m n)) ->
    (forall Γ (m : mfixpoint term) (n : nat), tFixProp (P Γ) (P (Γ ,,, fix_context m)) m -> P Γ (eCoFix m n)) ->
    (forall Γ (e : err_type) (A : term), P Γ A -> P Γ (eErr e A)) ->
    (forall Γ (A B t : term), P Γ A -> P Γ B -> P Γ t -> P Γ (eCast A B t)) ->
...
Fixpoint subst_app (t : term) (us : list term) : term :=
  match t, us with
  | eLambda _ A t, u :: us => subst_app (t {0 := u}) us
  | _, [] => t
  | _, _ => mkApps t us
  end.
