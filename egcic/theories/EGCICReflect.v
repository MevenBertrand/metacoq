...
From MetaCoq.EGCIC Require Import EGCICAst EGCICInduction.
...
(** Some properties of the newly introduced error type *)

Derive NoConfusion NoConfusionHom for err_type.

Definition eq_errtype e e' :=
  match e, e' with
  | ukn, ukn | rai, rai => true
  | _, _ => false
  end.

#[program] Instance reflect_errtype : ReflectEq err_type := {
  eqb := eq_errtype
}.
Next Obligation.
  ...
  all: constructor.
  all: try reflexivity.
  all: now discriminate.
Defined.

Lemma eq_errtype_refl e : eq_errtype e e.
Proof.
  destruct e.
  all: now unfold eq_inductive, eqb; cbn.
Defined.
...
Local Ltac term_dec_tac term_dec :=
  repeat match goal with
         ...
         | x : Instance.t, y : Instance.t |- _ =>
           fcase (eq_dec x y)
         | x : Level.t, y : Level.t |- _ => fcase (eq_dec x y)
         ...
         | e : err_type, e' : err_type |- _ => fcase (eq_dec e e')
         end.
