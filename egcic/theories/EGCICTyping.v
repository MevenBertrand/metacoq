...
EGCICLiftSubst EGCICUnivSubst EGCICEquality EGCICPosition EGCICConversion.
...
(** * Typing derivations
  ...
  Inductive relations for reduction, conversion and typing of EGCIC terms.
...
Fixpoint isArity T :=
  match T with
  | eSort u => True
  | eProd _ _ codom => isArity codom
  | eLetIn _ _ _ codom => isArity codom
  | _ => False
  end.
...
Lemma inds_spec ind u l :
  inds ind u l = List.rev (mapi (fun i _ => eInd {| inductive_mind := ind; inductive_ind := i |} u) l).
Proof.
...
Fixpoint destArity Γ (t : term) :=
  match t with
  | eProd na t b => destArity (Γ ,, vass na t) b
  | eLetIn na b b_ty b' => destArity (Γ ,, vdef na b b_ty) b'
  | eSort s => Some (Γ, s)
  | _ => None
  end.
...
Lemma mkApps_nonempty f l :
  l <> [] -> mkApps f l = eApp (mkApps f (removelast l)) (last l f).
Proof.
...
Lemma destArity_eFix {mfix idx args} :
  destArity [] (mkApps (eFix mfix idx) args) = None.
Proof.
...
Lemma destArity_eApp {t u l} :
  destArity [] (mkApps (eApp t u) l) = None.
Proof.
...
Lemma destArity_eInd {t u l} :
  destArity [] (mkApps (eInd t u) l) = None.
Proof.
  induction l. reflexivity.
  rewrite mkApps_nonempty.
  intros e; discriminate e.
...
Section WfArity.
  ...
  Lemma isWfArity_Sort Σ Γ u : All_local_env (lift_typing typing Σ) Γ -> isWfArity Σ Γ (eSort u).
  Proof.
    intros.
    eexists. eexists.
    split.
...
(* Axiom fix_guard_red1 :
  forall Σ Γ mfix mfix' idx,
    fix_guard mfix ->
    red1 Σ Γ (eFix mfix idx) (eFix mfix' idx) ->
    fix_guard mfix'. *)

Axiom fix_guard_eq_term :
  forall mfix mfix' idx,
    fix_guard mfix ->
    upto_names (eFix mfix idx) (eFix mfix' idx) ->
    fix_guard mfix'.
...
forall Σ Γ mfix mfix' idx,
  cofix_guard mfix ->
  red1 Σ Γ (eCoFix mfix idx) (eCoFix mfix' idx) ->
  cofix_guard mfix'.

Axiom cofix_guard_eq_term :
forall mfix mfix' idx,
  cofix_guard mfix ->
  upto_names (eCoFix mfix idx) (eCoFix mfix' idx) ->
  cofix_guard mfix'.
...
Fixpoint instantiate_params_subst params pars s ty :=
  ...
  | d :: params =>
    ...
    | Some b, eLetIn _ _ _ b' => instantiate_params_subst params pars (subst0 s b :: s) b'
...
Definition build_branches_type ind mdecl idecl params u p : list (option (nat × term)) :=
  ...
  let branch_type i '(id, t, ar) :=
    ...
    | Some ty =>
      ...
      let cstr := eConstruct ind i u in
...
Definition build_case_predicate_type ind mdecl idecl params u ps : option term :=
  ...
  let inddecl :=
      {| decl_name := nNamed idecl.(ind_name);
         decl_body := None;
         decl_type := mkApps (eInd ind u) (map (lift0 #|X.1|) params ++ to_extended_list X.1) |} in
  ret (it_mkProd_or_LetIn (X.1 ,, inddecl) (eSort ps)).

Definition destInd (t : term) :=
match t with
| eInd ind u => Some (ind, u)
| _ => None
end.
...
| type_Rel n decl :
    ...
    Σ ;;; Γ |- eRel n : lift0 (S n) decl.(decl_type)

| type_Sort l :
    All_local_env (lift_typing typing Σ) Γ ->
    LevelSet.In l (global_ext_levels Σ) ->
    Σ ;;; Γ |- eSort (Universe.make l) : eSort (Universe.super l)

| type_Prod na A B s1 s2 :
    Σ ;;; Γ |- A : eSort s1 ->
    Σ ;;; Γ ,, vass na A |- B : eSort s2 ->
    Σ ;;; Γ |- eProd na A B : eSort (Universe.sort_of_product s1 s2)

| type_Lambda na A t s1 B :
    Σ ;;; Γ |- A : eSort s1 ->
    Σ ;;; Γ ,, vass na A |- t : B ->
    Σ ;;; Γ |- (eLambda na A t) : eProd na A B

| type_LetIn na b B t s1 A :
    Σ ;;; Γ |- B : eSort s1 ->
    Σ ;;; Γ |- b : B ->
    Σ ;;; Γ ,, vdef na b B |- t : A ->
    Σ ;;; Γ |- eLetIn na b B t : eLetIn na b B A
...
| type_CoFix mfix n decl :
    ...
    Σ ;;; Γ |- eCoFix mfix n : decl.(dtype)

| type_Err e A s :
    Σ ;;; Γ |- A : eSort s ->
    Σ ;;; Γ |- eErr e A : A

| type_Cast A B t s1 s2 :
    Σ ;;; Γ |- A : eSort s1 ->
    Σ ;;; Γ |- B : eSort s2 ->
    Σ ;;; Γ |- A ~~ B ->
    Σ ;;; Γ |- t : A ->
...
| type_Cumul t A B :
    Σ ;;; Γ |- t : A ->
    (isWfArity typing Σ Γ B + {s & Σ ;;; Γ |- B : eSort s})%type ->
    Σ ;;; Γ |- A = B -> Σ ;;; Γ |- t : B
...
Module EGCICTypingDef <: Typing EGCICTerm EGCICEnvironment EGCICEnvTyping.
...
Module EGCICDeclarationTyping :=
  DeclarationTyping
    ...
    EGCICTypingDef
    EGCICLookup.
Include EGCICDeclarationTyping.
...
Proof.
  ...
  all: destruct s as [s | [u Hu]]; try lia.
  all: destruct s0 as [s0 | [u0 Hu0]] ; try lia.
Qed.
...
Lemma type_Prop_wf `{checker_flags} Σ Γ : wf_local Σ Γ -> Σ ;;; Γ |- eSort Universe.type0m : eSort Universe.type1.
Proof. 
...
Lemma type_Prop `{checker_flags} Σ : Σ ;;; [] |- eSort Universe.type0m : eSort Universe.type1.
  repeat constructor.
  apply prop_global_ext_levels.
Defined.
 ...
 *)
 ...
 forall (P : global_env_ext -> context -> term -> term -> Type)
   ...
   (forall Σ (wfΣ : wf Σ.1) (Γ : context) (wfΓ : wf_local Σ Γ) (n : nat) decl,
       nth_error Γ n = Some decl ->
       PΓ Σ Γ wfΓ ->
       P Σ Γ (eRel n) (lift0 (S n) decl.(decl_type))) ->
   (forall Σ (wfΣ : wf Σ.1) (Γ : context) (wfΓ : wf_local Σ Γ) (l : Level.t),
       PΓ Σ Γ wfΓ ->
       LevelSet.In l (global_ext_levels Σ) ->
       P Σ Γ (eSort (Universe.make l)) (eSort (Universe.super l))) ->

   (forall Σ (wfΣ : wf Σ.1) (Γ : context) (wfΓ : wf_local Σ Γ) (n : name) (t b : term) (s1 s2 : Universe.t),
       PΓ Σ Γ wfΓ ->
       Σ ;;; Γ |- t : eSort s1 ->
       P Σ Γ t (eSort s1) ->
       Σ ;;; Γ,, vass n t |- b : eSort s2 ->
       P Σ (Γ,, vass n t) b (eSort s2) -> P Σ Γ (eProd n t b) (eSort (Universe.sort_of_product s1 s2))) ->
   ...
   (forall Σ (wfΣ : wf Σ.1) (Γ : context) (wfΓ : wf_local Σ Γ) (n : name) (t b : term)
       ...
       Σ ;;; Γ |- t : eSort s1 ->
       P Σ Γ t (eSort s1) ->
       Σ ;;; Γ,, vass n t |- b : bty -> P Σ (Γ,, vass n t) b bty -> P Σ Γ (eLambda n t b) (eProd n t bty)) ->
   ...
   (forall Σ (wfΣ : wf Σ.1) (Γ : context) (wfΓ : wf_local Σ Γ) (n : name) (b b_ty b' : term)
       ...
       P Σ (Γ,, vdef n b b_ty) b' b'_ty -> P Σ Γ (eLetIn n b b_ty b') (eLetIn n b b_ty b'_ty)) ->

   (forall Σ (wfΣ : wf Σ.1) (Γ : context) (wfΓ : wf_local Σ Γ) (t : term) na A B u,
       PΓ Σ Γ wfΓ ->
       Σ ;;; Γ |- t : eProd na A B -> P Σ Γ t (eProd na A B) ->
       Σ ;;; Γ |- u : A -> P Σ Γ u A ->
       P Σ Γ (eApp t u) (B{0 := u})) ->
   ...
   (forall Σ (wfΣ : wf Σ.1) (Γ : context) (wfΓ : wf_local Σ Γ) (cst : kername) u (decl : constant_body),
       ...
       consistent_instance_ext Σ decl.(cst_universes) u ->
       P Σ Γ (eConst cst u) (subst_instance_constr u (cst_type decl))) ->
   ...
   (forall Σ (wfΣ : wf Σ.1) (Γ : context) (wfΓ : wf_local Σ Γ) (ind : inductive) u
       ...
       PΓ Σ Γ wfΓ ->
       consistent_instance_ext Σ mdecl.(ind_universes) u ->
       P Σ Γ (eInd ind u) (subst_instance_constr u (ind_type idecl))) ->
   ...
   (forall Σ (wfΣ : wf Σ.1) (Γ : context) (wfΓ : wf_local Σ Γ) (ind : inductive) (i : nat) u
       ...
       PΓ Σ Γ wfΓ ->
       consistent_instance_ext Σ mdecl.(ind_universes) u ->
       P Σ Γ (eConstruct ind i u) (type_of_constructor mdecl cdecl (ind, i) u)) ->
   ...
   (forall Σ (wfΣ : wf Σ.1) (Γ : context) (wfΓ : wf_local Σ Γ) (ind : inductive) u (npar : nat)
       ...
       Σ ;;; Γ |- c : mkApps (eInd ind u) args ->
       isCoFinite mdecl.(ind_finite) = false ->
       P Σ Γ c (mkApps (eInd ind u) args) ->
       forall btys, map_option_out (build_branches_type ind mdecl idecl params u p) = Some btys ->
       All2 (fun br bty => (br.1 = bty.1) *
                        (Σ ;;; Γ |- br.2 : bty.2) * P Σ Γ br.2 bty.2 *
                        (Σ ;;; Γ |- bty.2 : eSort ps) * P Σ Γ bty.2 (eSort ps))
            brs btys ->
       P Σ Γ (eCase (ind, npar) p c brs) (mkApps p (skipn npar args ++ [c]))) ->
   ...
   (forall Σ (wfΣ : wf Σ.1) (Γ : context) (wfΓ : wf_local Σ Γ) (p : projection) (c : term) u
       ...
       Forall_decls_typing P Σ.1 -> PΓ Σ Γ wfΓ ->
       Σ ;;; Γ |- c : mkApps (eInd (fst (fst p)) u) args ->
       P Σ Γ c (mkApps (eInd (fst (fst p)) u) args) ->
       #|args| = ind_npars mdecl ->
       let ty := snd pdecl in P Σ Γ (eProj p c) (subst0 (c :: List.rev args) (subst_instance_constr u ty))) ->
   ...
   (forall Σ (wfΣ : wf Σ.1) (Γ : context) (wfΓ : wf_local Σ Γ) (mfix : list (def term)) (n : nat) decl,
       ...
       nth_error mfix n = Some decl ->
       PΓ Σ Γ wfΓ ->
       All (fun d => {s & (Σ ;;; Γ |- d.(dtype) : eSort s)%type * P Σ Γ d.(dtype) (eSort s)})%type mfix ->
   ...
   (forall Σ (wfΣ : wf Σ.1) (Γ : context) (wfΓ : wf_local Σ Γ) (mfix : list (def term)) (n : nat) decl,
       ...
       All (fun d => (Σ ;;; Γ ,,, types |- d.(dbody) : lift0 #|types| d.(dtype))%type *
           P Σ (Γ ,,, types) d.(dbody) (lift0 #|types| d.(dtype)))%type mfix ->
       wf_cofixpoint Σ.1 mfix ->
       P Σ Γ (eCoFix mfix n) decl.(dtype)) ->

  (forall Σ (wfΣ : wf Σ.1) (Γ : context) (wfΓ : wf_local Σ Γ) (e : err_type) (A : term) (s : Universe.t),
       PΓ Σ Γ wfΓ ->
       Σ ;;; Γ |- A : eSort s ->
       P Σ Γ A (eSort s) ->
       P Σ Γ (eErr e A) A) ->

    (forall Σ (wfΣ : wf Σ.1) (Γ : context) (wfΓ : wf_local Σ Γ) (t A B : term) (s1 s2 : Universe.t),
        PΓ Σ Γ wfΓ ->
        Σ ;;; Γ |- A : eSort s1 -> P Σ Γ A (eSort s1) ->
        Σ ;;; Γ |- B : eSort s2 -> P Σ Γ B (eSort s2) ->
        Σ ;;; Γ |- A ~~ B ->
        Σ ;;; Γ |- t : A ->
        P Σ Γ t A ->
        P Σ Γ (eCast A B t) B) ->
    ...
    (forall Σ (wfΣ : wf Σ.1) (Γ : context) (wfΓ : wf_local Σ Γ) (t A B : term),
        ...
        Σ ;;; Γ |- t : A ->
        P Σ Γ t A ->
        (isWfArity_prop typing Pdecl Σ Γ B + {s & (Σ ;;; Γ |- B : eSort s) * P Σ Γ B (eSort s)})%type ->
        Σ ;;; Γ |- A = B ->
        P Σ Γ t B) ->
...
Proof.
  ...
  - simple refine (let IH' := IH ((Σ, udecl); (X15; []; (eSort Universe.type0m ); _; _)) in _).
  ...
  - simpl. simpl in *.
    ...
    + destruct c; simpl in *.
      ...
      specialize (IH (existT _ (Σ, udecl) (existT _ X15 (existT _ [] (existT _ _ (existT _ _ Xg)))))).
      simpl in IH.
      forward IH. constructor 1. simpl; lia.
      apply IH.
      red. simpl. red in Xg; simpl in Xg.
      destruct Xg as [s Hs]. red. simpl.
      specialize (IH (existT _ (Σ, udecl) (existT _ X15 (existT _ [] (existT _ _ (existT _ _ Hs)))))).
      simpl in IH.
    ...
    + red in Xg.
      ...
      ++ pose proof Xg.(onConstructors) as Xg'.
         ...
         destruct X16 as [cass chead tyeq onctyp oncargs oncind].
         unshelve econstructor ; eauto.
         destruct onctyp as [s Hs].
         simpl in Hs.
         specialize (IH (existT _ (Σ, udecl) (existT _ X15 (existT _ _ (existT _ _ (existT _ _ Hs)))))).
         simpl in IH. simpl. exists s. simpl. apply IH; constructor 1; simpl; auto with arith.
         eapply type_local_ctx_impl; eauto. simpl. intros. red in X16.
         destruct T.
         specialize (IH ((Σ, udecl); (X15; _; _; _; X16))).
         apply IH. simpl. constructor 1. simpl. auto with arith.
         destruct X16 as [u Hu]. exists u.
         specialize (IH (existT _ (Σ, udecl) (existT _ X15 (existT _ _ (existT _ _ (existT _ _ Hu)))))).
         apply IH. simpl. constructor 1. simpl. auto with arith.
         clear -X15 IH oncind.
         revert oncind.
         ...
         red in p0 |- *.
         specialize (IH (existT _ (Σ, udecl) (existT _ X15 (existT _ _ (existT _ _ (existT _ _ p0)))))).
         apply IH. simpl. constructor 1. simpl. auto with arith.
      ...
      ++ red in onP |- *.
         ...
         intros. destruct T; simpl in X16.
         specialize (IH (existT _ (Σ, udecl) (existT _ X15 (existT _ _ (existT _ _ (existT _ _ X16)))))).
         simpl in IH. apply IH. constructor 1. simpl. lia.
         destruct X16 as [u Hu].
         specialize (IH (existT _ (Σ, udecl) (existT _ X15 (existT _ _ (existT _ _ (existT _ _ Hu)))))).
  ...
  - assert (forall Γ t T (Hty : Σ ;;; Γ |- t : T),
    ...
    - red. eapply (X16 _ _ _ Hu). lia.
    - simpl in *. apply IHfoo. lia.
    - red. apply (X16 _ _ _ t3). lia.
    - red. apply (X16 _ _ _ Hu). lia. }
    eapply XΓ; eauto.
  ...
  - assert (forall Γ t T (Hty : Σ ;;; Γ |- t : T),
    ...
    { specialize (X16 _ _ _  (type_Prop _)).
      simpl in X16. intros Hle. apply X16. lia. }
    ...
    { clear -Pdecl wfΣ X16.
      ...
      - red. apply (X16 _ _ _ t3). lia.
      - red. apply (X16 _ _ _ Hu). lia. }
    apply XΓ in X15. all: auto.

    destruct H;
      try solve [  match reverse goal with
                     H : _ |- _ => eapply H
                   end; eauto;
                   unshelve eapply X16; simpl; auto with arith].

    -- match reverse goal with
         H : _ |- _ => eapply H
       end; eauto;
         unshelve eapply X16; simpl; eauto with arith wf.
    ...
    -- destruct indnpar as [ind' npar'];
       ...
       eapply X8; eauto.
       ++ eapply (X16 _ _ _ H); eauto. simpl; auto with arith.
       ++ eapply (X16 _ _ _ H); eauto. simpl; auto with arith.
       ++ simpl in *.
          eapply (X16 _ _ _ H0); eauto. clear. lia.
       ++ clear X15 Hdecls. revert a X16. simpl. clear. intros.
          induction a; simpl in *.
          ** constructor.
          ** destruct r as [[? ?] ?]. constructor.
             --- intuition eauto.
                 +++ eapply (X16 _ _ _ t); eauto. simpl; auto with arith.
                     lia.
                 +++ eapply (X16 _ _ _ t0); eauto. simpl; auto with arith.
                     lia.
             --- apply IHa. auto. intros.
                 eapply (X16 _ _ _ Hty). lia.

    -- eapply X9; eauto.
       apply Hdecls; simpl.
       pose proof (typing_size_pos H). lia.
       eapply (X16 _ _ _  H). simpl. lia.
    ...
    -- clear X X0 X1 X2 X3 X4 X5 X6 X7 X8 X9 X11 X12.
       ...
       * assert(forall (t T : term) (Hty : Σ;;; Γ |- t : T),
                    ...
                    ∑ s : Universe.t, Σ;;; Γ |- dtype x : eSort s)
                     (fun (x : def term)
                     (p : ∑ s : Universe.t, Σ;;; Γ |- dtype x : eSort s) =>
                   typing_size p.π2) a0) ->
                   Forall_decls_typing P Σ.1 * P Σ Γ t T).
        intros; eauto. eapply (X16 _ _ _ Hty); eauto. lia.
        clear X15 X16 a Hdecls.
        clear -a0 X.
        induction a0; constructor.
        destruct p as [s Hs]. exists s; split; auto.
        apply (X (dtype x) (eSort s) Hs). simpl. lia.
        ...
        * simpl in X14.
        ...
        {intros. eapply (X16 _ _ _ Hty); eauto. simpl. lia. }
        clear X16 X15.
    ...
    -- eapply X14; eauto.
       ++ eapply (X16 _ _ _ H); simpl. destruct s as [s | [u Hu]]; lia.
       ++ simpl in X16, X13.
          destruct s as [s | [u Hu]].
          ** left.
             red. exists s. red in s. revert X16.
             generalize (snd (projT2 (projT2 s))).
             induction a; simpl in *; intros X16 *; constructor.
             --- apply IHa. intros. eapply (X16 _ _ _ Hty). lia.
             --- red. eapply (X16 _ _ _ (projT2 t1)). destruct t1. simpl. lia.
             --- apply IHa. intros. eapply (X16 _ _ _ Hty). lia.
             --- red. destruct t1. unshelve eapply X16. all: eauto.
                 simpl. lia.
             --- red. destruct t1. unshelve eapply X16. all: eauto.
                 simpl. lia.
          ** right.
             exists u. intuition eauto. unshelve eapply X16. all: eauto. lia.
Qed.
...
Section All_local_env.
  ...
  Proof.
    ...
    - eapply All_local_env_impl. 1: exact h2.
      intros Γ t [T|] h.
      all: cbn in *.
      all: change EGCICEnvironment.app_context with app_context in *.
  ...
  Proof.
    ...
    - eapply All_local_env_impl.
      + eassumption.
      + change EGCICEnvironment.app_context with app_context.
        intros Γ t []; cbn;
